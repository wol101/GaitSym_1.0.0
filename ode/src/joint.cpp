...
static void PUInit (dxJointPU *j)
{
  // Default Position
  //               Y                ^ Axis2
  //              ^                 |
...
dReal dJointGetPUPosition (dJointID j)
{
  ...
  dUASSERT (joint,"bad joint argument");
  dUASSERT (joint->vtable == &__dPU_vtable,"joint is not a Prismatic and Universal");
...
static void PUGetInfo1 (dxJointPU *j, dxJoint::Info1 *info)
{
  info->m = 3;
  info->nub = 3;
...
// powered needs an extra constraint row
  ...
  if ( (j->limotP.lostop > -dInfinity || j->limotP.histop < dInfinity) &&
       j->limotP.lostop <= j->limotP.histop) {
    // measure joint position
    dReal pos = dJointGetPUPosition (j);
    j->limotP.testRotationalLimit (pos); // N.B. The function is ill named
  }
...
static void PUGetInfo2 (dxJointPU *joint, dxJoint::Info2 *info)
{
  const int s0 = 0;
  ...
  if (joint->node[1].body) {
    ...
    dCROSS ( (info->J2a) +s1, = , ax1, anchor2);
    // The cross product is in reverse order since we want the negative value
    dCROSS ( (info->J2a) +s2, = , q, anchor2);
  ...
  // The position should be the same when we are not along the prismatic axis
  dVector3 err;
  dMULTIPLY0_331 (err, R1, joint->anchor1);
  // err[i] = dist[i] - err[i];
  dOPE2 (err, =, dist, -, err);
  info->c[1] = k * dDOT (ax1, err);
  info->c[2] = k * dDOT (q, err);

  int row = 3 + joint->limot1.addLimot (joint,info,3,ax1,1);
  row += joint->limot2.addLimot (joint,info,row,ax2,1);
  joint->limotP.addLimot (joint,info,row,axP,0);
}
...
dReal dJointGetPUAngle1 (dJointID j)
{
  dxJointUniversal* joint = (dxJointUniversal*) j;
  dUASSERT (joint,"bad joint argument");
  dUASSERT (joint->vtable == &__dPU_vtable,
...
dReal dJointGetPUParam (dJointID j, int parameter)
{
  dxJointPU* joint = (dxJointPU*) j;
  dUASSERT (joint,"bad joint argument");
  dUASSERT (joint->vtable == &__dPU_vtable,"joint is not Prismatic and Universal");
...
dxJoint::Vtable __dPU_vtable = {
                                 sizeof (dxJointPU),
                                 (dxJoint::init_fn*) PUInit,
                                 (dxJoint::getInfo1_fn*) PUGetInfo1,
                                 (dxJoint::getInfo2_fn*) PUGetInfo2,
                                 dJointTypePU
                               };
...
static void PistonInit (dxJointPiston *j)
{
  dSetZero (j->axis1,4);
  dSetZero (j->axis2,4);
...
dReal dJointGetPistonPosition (dJointID j)
{
  ...
  dUASSERT (joint,"bad joint argument");
  dUASSERT (joint->vtable == &__dPiston_vtable, "joint is not a Piston");
...
static void PistonGetInfo1 (dxJointPiston *j, dxJoint::Info1 *info)
{
  ...
  if ( (j->limotP.lostop > -dInfinity || j->limotP.histop < dInfinity) &&
       j->limotP.lostop <= j->limotP.histop) {
    // measure joint position
    dReal pos = dJointGetPistonPosition (j);
    j->limotP.testRotationalLimit (pos);       // N.B. The fucntion is ill named
  }
  ...
  // see if we're at a joint limit.
  j->limotR.limit = 0;
  if ( (j->limotR.lostop > -dInfinity || j->limotR.histop < dInfinity) &&
       j->limotR.lostop <= j->limotR.histop) {
    // measure joint position
    dReal angle = getHingeAngle (j->node[0].body, j->node[1].body, j->axis1,
                                 j->qrel);
    j->limotR.testRotationalLimit (angle);
  }

  // powered Piston or at limits needs an extra constraint row
  if (j->limotR.limit || j->limotR.fmax > 0) info->m++;

}


static void PistonGetInfo2 (dxJointPiston *joint, dxJoint::Info2 *info)
{
  const int s0 = 0;
  const int s1 = info->rowskip;
  ...
  if (joint->node[1].body) {
    ...
    dVector3 ax2;
    dMULTIPLY0_331 (ax2, R2, joint->axis2);
    dCROSS (b,=,ax1, ax2);
  }
  else {
    // Some math for the RHS
    dCROSS (b,=,ax1,joint->axis2);
  }
  ...
  int row = 4 + joint->limotP.addLimot (joint, info, 4, ax1, 0);
  joint->limotR.addLimot (joint,info,row,ax1,1);
}

static void PistonComputeInitialRelativeRotation  (dxJointPiston *joint)
{
  if (joint->node[0].body) {
    if (joint->node[1].body) {
      dQMultiply1 (joint->qrel,joint->node[0].body->q,joint->node[1].body->q);
...
void dJointSetPistonAnchor (dJointID j, dReal x, dReal y, dReal z)
{
  ...
  setAnchors (joint,x,y,z,joint->anchor1,joint->anchor2);
  PistonComputeInitialRelativeRotation (joint);
...
dxJoint::Vtable __dPiston_vtable = {
                                     sizeof (dxJointPiston),
                                     (dxJoint::init_fn*) PistonInit,
                                     (dxJoint::getInfo1_fn*) PistonGetInfo1,
                                     (dxJoint::getInfo2_fn*) PistonGetInfo2,
                                     dJointTypePiston};
...
//****************************************************************************
// floatingHinge (wis)
// just like a hinge without the linear constraints
// added by wis@mac.com Fri Dec 29 16:00:08 GMT 2006

static void floatingHingeInit (dxJointFloatingHinge *j)
{
    // dSetZero (j->anchor1,4);
    // dSetZero (j->anchor2,4);
    dSetZero (j->axis1,4);
...
static void floatingHingeGetInfo1 (dxJointFloatingHinge *j, dxJoint::Info1 *info)
{
    // I aught to take off 3 rows but this should get it working
...
static void floatingHingeGetInfo2 (dxJointFloatingHinge *joint, dxJoint::Info2 *info)
{
    // this is the linear bit that I don't need
    // set the three ball-and-socket rows
    // setBall (joint,info,joint->anchor1,joint->anchor2); // removed
    // so set the linear Jacobians to zero (I'm not sure if these are used)
    int r0 = 0;
    int r1 = info->rowskip;
    int r2 = 2 * r1;
    ...
    if (joint->node[1].body) 
    {
        VoXYZ (&info->J2l[r0], =, 0, 0, 0);
        VoXYZ (&info->J2l[r1], =, 0, 0, 0);
        VoXYZ (&info->J2l[r2], =, 0, 0, 0);
    }
    for (int j = 0; j < 3; j++) { info->c[j] = 0; } // and set the error to zero
...
static void hingeComputeInitialRelativeRotation (dxJointFloatingHinge *joint)
{
    if (joint->node[0].body) {
        if (joint->node[1].body) {
            dQMultiply1 (joint->qrel,joint->node[0].body->q,joint->node[1].body->q);
...
void dJointSetFloatingHingeAxis (dJointID j, dReal x, dReal y, dReal z)
{
    ...
    dUASSERT(joint,"bad joint argument");
    dUASSERT(joint->vtable == &__dfloatinghinge_vtable,"joint is not a floatinghinge");
    setAxes (joint,x,y,z,joint->axis1,joint->axis2);
    hingeComputeInitialRelativeRotation (joint);
}
...
dxJoint::Vtable __dfloatinghinge_vtable = {
    sizeof(dxJointFloatingHinge),
    (dxJoint::init_fn*) floatingHingeInit,
    (dxJoint::getInfo1_fn*) floatingHingeGetInfo1,
    (dxJoint::getInfo2_fn*) floatingHingeGetInfo2,
