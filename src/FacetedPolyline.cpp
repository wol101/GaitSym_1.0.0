...
void FacetedPolyline::Extrude(std::vector<pgd::Vector> *polyline, std::vector<pgd::Vector> *profile, std::vector<Polygon *> *faces)
{
    unsigned int i, j;
    Line3D line;
    ...
    pgd::Vector axis = zVec ^ v1;
    dReal angle = acos(zVec * v1);
    pgd::Quaternion q = pgd::MakeQFromAxis(axis.x, axis.y, axis.z, angle);
    ...
    faces->push_back(face);
    face = new Polygon();
    for (i = 0; i < rotatedProfile.size(); i++) face->vertices.push_back(vertexList[joinPlanes.size() - 1 + joinPlanes.size() * i]);
    faces->push_back(face);
