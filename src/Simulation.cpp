...
void Simulation::UpdateSimulation()
{
    ...
    for (iter3=m_DataTargetList.begin(); iter3 != m_DataTargetList.end(); iter3++)
    {
        const dReal *r;
        dReal v, w;
        if (iter3->second->GetTargetValue(m_SimulationTime, &v, &w))
        {
            dReal kinematicError = 0;
            switch (iter3->second->GetDataType())
            {
                case DataTarget::Q0:
                    r = iter3->second->GetTarget()->GetQuaternion();
                    kinematicError = r[0] - v;
                    break;
                case DataTarget::Q1:
                    r = iter3->second->GetTarget()->GetQuaternion();
                    kinematicError = r[1] - v;
                    break;
                case DataTarget::Q2:
                    r = iter3->second->GetTarget()->GetQuaternion();
                    kinematicError = r[2] - v;
                    break;
                case DataTarget::Q3:
                    r = iter3->second->GetTarget()->GetQuaternion();
                    kinematicError = r[3] - v;
                    break;
                case DataTarget::XP:
                    r = iter3->second->GetTarget()->GetPosition();
                    kinematicError = r[0] - v;
                    break;
                case DataTarget::YP:
                    r = iter3->second->GetTarget()->GetPosition();
                ...
                case DataTarget::ZP:
                    ...
                    break;
                case DataTarget::XV:
                    r = iter3->second->GetTarget()->GetAngularVelocity();
                    kinematicError = r[0] - v;
                    break;
                case DataTarget::YV:
                    r = iter3->second->GetTarget()->GetAngularVelocity();
...
void Simulation::ParseGlobal(xmlNodePtr cur)
{
    ...
    if (strcmp((char *)buf, "DistanceTravelled") == 0) 
    {
        m_FitnessType = DistanceTravelled;
        THROWIFZERO(buf = DoXmlGetProp(cur, (const xmlChar *)"DistanceTravelledBodyID"));
        m_DistanceTravelledBodyIDName = (char *)buf;
    }
    else 
    {
        if (strcmp((char *)buf, "KinematicMatch") == 0)
        {
            m_FitnessType = KinematicMatch;
        }
        else 
        {
            throw __LINE__;
...
void Simulation::ParseBody(xmlNodePtr cur)
{
    ...
    // offset used for graphics and recalculating mass properties
    ...
    THROWIFZERO(buf = DoXmlGetProp(cur, (const xmlChar *)"Scale"));
    GLfloat scale = Util::Double(buf);
    facetedObject->Scale(scale, scale, scale);
    const dReal *p = theBody->GetOffset();
    facetedObject->Move(p[0], p[1], p[2]);
...
void Simulation::ParseJoint(xmlNodePtr cur)
{
    ...
    else if (strcmp((const char *)buf, "FloatingHinge") == 0)
    {
        ...
        THROWIFZERO(buf = DoXmlGetProp(cur, (const xmlChar *)"StartAngleReference"));
        floatingHingeJoint->SetStartAngleReference(Util::Double(buf));
        THROWIFZERO(buf = DoXmlGetProp(cur, (const xmlChar *)"ParamLoStop"));
        dReal loStop = Util::Double(buf);
        THROWIFZERO(buf = DoXmlGetProp(cur, (const xmlChar *)"ParamHiStop"));
        dReal hiStop = Util::Double(buf);
...
void Simulation::ParseDataTarget(xmlNodePtr cur)
{
    ...
    THROWIFZERO(buf = DoXmlGetProp(cur, (const xmlChar *)"Target"));
    dataTarget->SetTarget(m_BodyList[(const char *)buf]);
        ...
        for (iter0 = m_TagContentsList.begin(); iter0 != m_TagContentsList.end(); iter0++)
        {
            ...
            if ((!xmlStrcmp(cur->name, (const xmlChar *)"JOINT")))
            {
                ...
                if (jp)
                {
                    ...
                    if (gDebug == ModelStateDebug)
                    {
                        jp->GetHingeAnchor(v);
                        dBodyGetPosRelPoint(jp->GetBody(1), v[0], v[1], v[2], result);
                        Body *body = (Body *)(dBodyGetData(jp->GetBody(1)));
                        sprintf(m_Buffer, "%s %.17g %.17g %.17g", body->GetName()->c_str(), result[0], result[1], result[2]);
                ...
                if (jpb)
                {
                    ...
                    if (gDebug == ModelStateDebug)
                    {
                        ...
                        Body *body = (Body *)(dBodyGetData(jpb->GetBody(1)));
                        sprintf(m_Buffer, "%s %.17g %.17g %.17g", body->GetName()->c_str(), result[0], result[1], result[2]);
