...
// note memory is allocated by this routine and will need to be relased elsewhere
void FacetedObject::CalculateTrimesh(StridedVertex **vertices, int *numVertices, int *vertexStride, StridedTri **tris, int *numTris, int *triStride)
{
    int i;
    ...
    *vertices = new StridedVertex[mNumVertices];
    *tris = new StridedTri[mNumFaces];
    
    for (i = 0; i < mNumVertices; i++)
    {
        (*vertices)[i].Vertex[0] = mVertexList[i]->x;
        (*vertices)[i].Vertex[1] = mVertexList[i]->y;
        (*vertices)[i].Vertex[2] = mVertexList[i]->z;
    }
    
    for (i = 0; i < mNumFaces; i++)
    {
        (*tris)[i].Indices[0] = mFaceList[i]->GetVertex(0);
        (*tris)[i].Indices[1] = mFaceList[i]->GetVertex(1);
        (*tris)[i].Indices[2] = mFaceList[i]->GetVertex(2);
    }
}
...
void FacetedObject::CalculateMassProperties(dMass *m, dReal density, bool clockwise)
{
    ...
    // assumes clockwise winding so need to reverse if anticlockwise
    if (clockwise == false) for (int index = 0; index < mNumFaces; index++) mFaceList[index]->ReverseVertexOrder();
